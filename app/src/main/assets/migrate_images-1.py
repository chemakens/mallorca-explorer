...
def asset_to_storage_url(asset_url: str) -> str:
    """Convierte file:///android_asset/images/foo.jpg → URL de Firebase Storage (webp)."""
    filename = asset_url.replace(ASSET_PREFIX, "")
    # Las imágenes en Storage están en .webp aunque Firestore las tenga como .jpg/.jpeg
    for ext in [".jpg", ".jpeg"]:
        if filename.endswith(ext):
            filename = filename[:-len(ext)] + ".webp"
            break
